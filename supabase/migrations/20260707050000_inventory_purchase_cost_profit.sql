...
alter table public.sales
  add column if not exists purchase_cost_at_sale numeric(12, 2) not null default 0;

alter table public.sales
  add column if not exists profit_amount numeric(14, 2) generated always as (
    round((selling_price_per_unit - purchase_cost_at_sale) * quantity_sold::numeric, 2)
  ) stored;
...
begin
  ...
  values (
    p_company_id,
    'New sale recorded',
    format('%s sold %s units of "%s" at %s per unit (profit %s).',
      coalesce(actor_name, 'A user'),
      p_quantity_sold,
      target_item.title,
      p_selling_price_per_unit::text,
      recorded_sale.profit_amount::text
    ),
    actor_id,
    actor_id
  );
